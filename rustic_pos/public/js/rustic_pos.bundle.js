...
rustic_pos.onPOSReady = function() {
    ...
    // Observe for payment screen to add payment reference field
    rustic_pos.observePaymentScreen();

    // Observe for invoice type indicator
    rustic_pos.observeInvoiceType();
...
rustic_pos.observePaymentScreen = function() {
    ...
    if (posApp) {
        observer.observe(posApp, { childList: true, subtree: true, attributes: true });
    }
};

/**
 * Add payment reference field to payment screen (above Complete Order button)
 */
rustic_pos.addPaymentReferenceToPaymentScreen = function($paymentContainer) {
                    ...
                    <div class="form-group" style="margin-bottom: 0;">
                        <div class="control-input-wrapper">
                            <div class="control-input">
                                <input type="text"
                                    autocomplete="off"
                                    class="input-with-feedback form-control rustic-payment-ref-input"
                                    data-fieldtype="Data"
...
/**
 * Observe and display invoice type indicator (Sell Invoice / Return Invoice)
 */
rustic_pos.observeInvoiceType = function() {
    const observer = new MutationObserver(function(mutations) {
        rustic_pos.updateInvoiceTypeIndicators();
    });

    const posApp = document.querySelector('.point-of-sale-app');
    if (posApp) {
        observer.observe(posApp, { childList: true, subtree: true });
    }
...
/**
 * Update invoice type indicators in all locations
 */
rustic_pos.updateInvoiceTypeIndicators = function() {
    if (!window.cur_pos || !window.cur_pos.frm || !window.cur_pos.frm.doc) return;

    const isReturn = window.cur_pos.frm.doc.is_return;
    const typeLabel = isReturn ? __('Return Invoice') : __('Sell Invoice');
    const typeColor = isReturn ? '#ef4444' : '#22c55e';
    const typeBgColor = isReturn ? '#fef2f2' : '#f0fdf4';

    // 1. Cart Area - above cart items
    rustic_pos.addInvoiceTypeToCart(typeLabel, typeColor, typeBgColor, isReturn);

    // 2. Payment Screen - above Complete Order button
    rustic_pos.addInvoiceTypeToPayment(typeLabel, typeColor, typeBgColor, isReturn);

    // 3. Invoice Summary - near invoice name
    rustic_pos.addInvoiceTypeToSummary(typeLabel, typeColor, typeBgColor, isReturn);
};

/**
 * Add invoice type indicator to cart area
 */
rustic_pos.addInvoiceTypeToCart = function(typeLabel, typeColor, typeBgColor, isReturn) {
    const $cart = $('.point-of-sale-app .cart-container');
    if (!$cart.length) return;

    let $indicator = $cart.find('.rustic-invoice-type-cart');

    if (!$indicator.length) {
        const indicatorHtml = `
            <div class="rustic-invoice-type-cart" style="
                padding: 8px 12px;
                margin-bottom: 8px;
                background: ${typeBgColor};
                border-inline-start: 4px solid ${typeColor};
                border-radius: 4px;
                font-weight: 600;
                color: ${isReturn ? '#991b1b' : '#166534'};
            ">${typeLabel}</div>
        `;
        $cart.prepend(indicatorHtml);
    } else {
        // Update existing indicator
        $indicator.text(typeLabel)
            .css({
                'background': typeBgColor,
                'border-inline-start-color': typeColor,
                'color': isReturn ? '#991b1b' : '#166534'
            });
    }
};
...
/**
 * Add invoice type indicator to payment screen
 */
rustic_pos.addInvoiceTypeToPayment = function(typeLabel, typeColor, typeBgColor, isReturn) {
    const $payment = $('.point-of-sale-app .payment-container:visible');
    if (!$payment.length) return;

    let $indicator = $payment.find('.rustic-invoice-type-payment');
    const $submitBtn = $payment.find('.submit-order-btn');

    if (!$indicator.length && $submitBtn.length) {
        const indicatorHtml = `
            <div class="rustic-invoice-type-payment" style="
                padding: 12px;
                margin-bottom: 12px;
                background: ${typeBgColor};
                border-inline-start: 4px solid ${typeColor};
            ...
            ">${typeLabel}</div>
        `;
        $payment.find('.rustic-payment-reference').before(indicatorHtml);
...
/**
 * Add invoice type indicator to invoice summary
 */
rustic_pos.addInvoiceTypeToSummary = function(typeLabel, typeColor, typeBgColor, isReturn) {
    const $summary = $('.point-of-sale-app .abs-container');
    if (!$summary.length || !$summary.find('.invoice-name').length) return;

    let $indicator = $summary.find('.rustic-invoice-type-summary');
    const $upperSection = $summary.find('.upper-section');

    if (!$indicator.length && $upperSection.length) {
        const indicatorHtml = `
            <div class="rustic-invoice-type-summary" style="
                padding: 8px 12px;
                margin-bottom: 12px;
                background: ${typeBgColor};
                border-inline-start: 4px solid ${typeColor};
            ...
            ">${typeLabel}</div>
        `;
        $upperSection.after(indicatorHtml);
    } else if ($indicator.length) {
        // Update existing indicator
        $indicator.text(typeLabel)
            .css({
